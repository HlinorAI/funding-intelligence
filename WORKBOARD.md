...
- Added `ops/k18/run_contour.py`: one local command running searches, enrichment, qualification, a fail-closed dedup view, and a full-history reply check into `drafts/k18-local/contour-status.json`. Recipient discovery covers manifests plus Sent history from the K-18 alias (pilot subjects included, K16 traffic excluded); transport failures retry with cooldown. Live run 2026-09-16: 18 K-18 recipients, zero replies, per-address dedup verdicts for all five operator candidates.
- Wired Zoho review drafts into the local contour: `create_zoho_drafts.py` now reads the local gitignored env file, and the contour run appends drafts to the Zoho Drafts folder only under `K18_DRAFTS_MODE=enabled`. First enabled run on 2026-09-16 created one EBN draft (info@ebn.eu, idempotency-keyed) after the dedup gates refused CPA.com and EIT Urban Mobility repeat recipients; EBN was added to the allowlisted official sources as an active programme network with a public general email.
