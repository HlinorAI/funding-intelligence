...
See [benchmark methodology](docs/benchmark-methodology.md) and [`benchmarks/`](benchmarks/) for the source-bound expectations. The current suite contains seven completed public-only cases; planned cases are not counted as evidence.
...
AI program cards carry human-readable `evidence_requirements` and executable mechanism-specific `evidence_policy` contracts. Cloud-credit routes check account, legal, geography and workload fields; BD routes check a named workload, champion or buyer and an outcome; accelerator/investment routes check founder, market, product and traction fields; NVIDIA routes require a native GPU/NVIDIA use case. Cards without an executable policy use the legacy human-readable fallback and are reported as such.
